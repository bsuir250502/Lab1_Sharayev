...
date_t read_date(int allow_zero)
{
    ...
    day = input_number_in_range(1,31, BAN_ZERO);
...
int scan_tax(company_t * comp, int n)
{
    ...
    for (i = 0; i < n; i++) {
		...
		comp[i].tax = input_number_in_range(0, 1000000, BAN_ZERO);
    }
...
int scan_dates(company_t * comp, int n)
{
    ...
    for (i = 0; i < n; i++) {
        printf("Set the latest date of tax payment for company \"%s\":\n",
               comp[i].name);
        comp[i].last_date = read_date(BAN_ZERO);
      
    }
    for (i = 0; i < n; i++) {
        printf("Set the actual date for tax payment for company \"%s\":\n",
               comp[i].name);
        comp[i].payment_date = read_date(ALLOW_ZERO);
    }
